...
"Usage: mipi10_to_unpacked <source image data file> <output image data file>\n"
"\n"
"Converts a single-channel MIPI10 image into an unpacked 16-bit format.\n";

static const char *PROGRAM_SOURCE[] = {
"__kernel void unpack(__read_only  image2d_t packed_image,\n",
"                     __write_only image2d_t unpacked_image,\n",
"                                  sampler_t sampler)\n",
"{\n",
...
"    const float4 pixels[] = {\n",
"        read_imagef(packed_image, sampler, coord + (int2)(0, 0)),\n",
"        read_imagef(packed_image, sampler, coord + (int2)(1, 0)),\n",
"        read_imagef(packed_image, sampler, coord + (int2)(2, 0)),\n",
"        read_imagef(packed_image, sampler, coord + (int2)(3, 0)),\n",
"    };\n",
"    write_imagef(unpacked_image, coord + (int2)(0, 0), pixels[0]);\n",
"    write_imagef(unpacked_image, coord + (int2)(1, 0), pixels[1]);\n",
"    write_imagef(unpacked_image, coord + (int2)(2, 0), pixels[2]);\n",
"    write_imagef(unpacked_image, coord + (int2)(3, 0), pixels[3]);\n",
"}\n"
};
...
int main(int argc, char** argv)
{
    ...
    cl_kernel            kernel               = wrapper.make_kernel("unpack", program);
    ...
    out_format.image_channel_data_type = CL_UNORM_INT16;
    ...
    single_channel_int16_image_t out_image_info;
    out_image_info.width  = out_desc.image_width;
    out_image_info.height = out_desc.image_height;
    out_image_info.pixels.resize((out_image_info.width * 2) * out_image_info.height);
    ...
    {
        std::memcpy(
                out_image_info.pixels.data() + i * out_desc.image_width * 2,
                image_ptr                    + i * row_pitch,
                out_desc.image_width * 2
        );
    }
    ...
    save_single_channel_image_data(out_image_filename, out_image_info);
