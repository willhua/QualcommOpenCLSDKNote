//--------------------------------------------------------------------------------------
// File: qcom_block_match_ssdf.cpp
// Desc:
//
// Author:      QUALCOMM
//
...
static const char *HELP_MESSAGE = "\n"
        "Usage: qcom_block_match_ssdf <source image data file> <block size> <output image data file>\n"
        "\n"
        "Runs a kernel demonstrating qcom_block_match_ssdf by tiling the upper left sub-square of an image\n"
        "across every pixel in the input image and computing the sum of squared differences (SSD).\n"
        "Each pixel in the output image encodes the SSD for the block that originated at the same coordinates\n"
        "in the input image, normalized by the area of the block so that the resulting value is in the range [0, 1],\n"
        "and with the square root taken to \"flatten\" the distribution of values.\n"
...
static const char *PROGRAM_SOURCE[] = {
        "__kernel void block_match_ssd(__read_only     image2d_t               src_image,\n",
        ...
        "    float4 ssd   = qcom_block_match_ssdf(\n",
        "                       src_image,\n",
        "                       sampler,\n",
        "                       (float2)(wid_x, wid_y),\n",
        "                       region,\n",
        ...
        "                       (uint2)(0, 0)\n",
        "                   );\n",
        "    ssd /= region.x * region.y;\n",
        "    ssd  = sqrt(ssd);\n",
        "    write_imagef(dest_image, (int2)(wid_x, wid_y), ssd);\n",
        "}\n"
};
...
int main(int argc, char** argv)
{
    ...
    cl_kernel    y_plane_kernel      = wrapper.make_kernel("block_match_ssd", program);
    cl_kernel    uv_plane_kernel     = wrapper.make_kernel("block_match_ssd", program);
    ...
    {
        std::cerr << "Extension cl_qcom_accelerated_image_ops needed for qcom_block_match_ssdf is not supported.\n";
        std::exit(EXIT_FAILURE);
    }
