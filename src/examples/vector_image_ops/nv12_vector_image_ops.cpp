//--------------------------------------------------------------------------------------
// File: nv12_vector_image_ops.cpp
// Desc:
//
// Author:      QUALCOMM
//
...
"Usage: nv12_vector_image_ops <source image data file>\n"
"\n"
...
"       };\n",
"    qcom_write_imagefv_2x1_n8n00(dest_image_y_plane, write_coord,                y_pixels_out[0]);\n",
"    qcom_write_imagefv_2x1_n8n00(dest_image_y_plane, write_coord + (int2)(0, 1), y_pixels_out[1]);\n",
"}\n",
"\n",
...
"}\n",
"\n",
// Reads 4x1 from a YUV image and writes 4x1 to a Y-only image
"__kernel void read_yuv_4x1_write_y_4x1(__read_only  image2d_t src_image,\n",
"                                       __write_only image2d_t dest_image_y_plane,\n",
"                                                    sampler_t sampler)\n",
"{\n",
"    const int    wid_x          = get_global_id(0);\n",
"    const int    wid_y          = get_global_id(1);\n",
"    const float2 read_coord     = (float2)(4 * wid_x, wid_y) + 0.5;\n",
"    const int2   write_coord    = (int2)(4 * wid_x, wid_y);\n",
"    const float4 y_pixels_in    = qcom_read_imagef_4x1(src_image, sampler, read_coord, YUV_Y_PLANE);\n",
"    float        y_pixels_out[] = {y_pixels_in.s0, y_pixels_in.s1, y_pixels_in.s2, y_pixels_in.s3};\n",
"    qcom_write_imagefv_4x1_n8n00(dest_image_y_plane, write_coord, y_pixels_out);\n",
"}\n",
"\n",
// Reads 4x1 from a YUV image and writes 2x1 to a UV-only image
"__kernel void read_yuv_4x1_write_uv_2x1(__read_only  image2d_t src_image,\n",
"                                        __write_only image2d_t dest_image_uv_plane,\n",
"                                                     sampler_t sampler)\n",
"{\n",
"    const int    wid_x            = get_global_id(0);\n",
"    const int    wid_y            = get_global_id(1);\n",
"    const float2 read_coord       = (float2)(4 * wid_x, 2 * wid_y) + 0.5;\n",
"    const int2   write_coord      = (int2)(2 * wid_x, wid_y);\n",
"    const float4 u_pixels_in      = qcom_read_imagef_4x1(src_image, sampler, read_coord, YUV_U_PLANE);\n",
"    const float4 v_pixels_in      = qcom_read_imagef_4x1(src_image, sampler, read_coord, YUV_V_PLANE);\n",
"    float2       uv_pixels_out[2] = {\n",
"       {u_pixels_in.s0, v_pixels_in.s0}, {u_pixels_in.s2, v_pixels_in.s2},\n",
        "};\n",
"    qcom_write_imagefv_2x1_n8n01(dest_image_uv_plane, write_coord, uv_pixels_out);\n",
"}\n",
"\n",
...
"        {{u_pixels_in.s0, v_pixels_in.s0}, {u_pixels_in.s1, v_pixels_in.s1}},\n",
"        };\n",
"    qcom_write_imagefv_2x1_n8n01(dest_image_uv_plane, write_coord,                uv_pixels_out[0]);\n",
"    qcom_write_imagefv_2x1_n8n01(dest_image_uv_plane, write_coord + (int2)(0, 1), uv_pixels_out[1]);\n",
"}\n",
"\n",
// Reads 4x1 from a Y-only image and writes 4x1 to a Y-only image
"__kernel void read_y_4x1_write_y_4x1(__read_only  image2d_t src_image_y_plane,\n",
"                                     __write_only image2d_t dest_image_y_plane,\n",
"                                                  sampler_t sampler)\n",
"{\n",
"    const int    wid_x          = get_global_id(0);\n",
...
"    const float2 read_coord     = (float2)(4 * wid_x, wid_y) + 0.5;\n",
"    const int2   write_coord    = (int2)(4 * wid_x, wid_y);\n",
"    const float4 y_pixels_in    = qcom_read_imagef_4x1(src_image_y_plane, sampler, read_coord, Y_Y_PLANE);\n",
"    float        y_pixels_out[] = {y_pixels_in.s0, y_pixels_in.s1, y_pixels_in.s2, y_pixels_in.s3};\n",
"    qcom_write_imagefv_4x1_n8n00(dest_image_y_plane, write_coord, y_pixels_out);\n",
"}\n"
};
...
int main(int argc, char** argv)
{
    ...
    cl_kernel    copy_kernels[]      = {
            ...
            wrapper.make_kernel("read_yuv_4x1_write_y_4x1",  program),
            wrapper.make_kernel("read_yuv_4x1_write_uv_2x1", program),
            wrapper.make_kernel("read_uv_2x2_write_uv_2x1",  program),
            wrapper.make_kernel("read_y_4x1_write_y_4x1",  program),
    };
    ...
    std::array<kernel_exec_params_t, 6> kernel_execution_params{
            /*                   kernel,          source,         destination,  work_size[0],                             work_size[1] */
            kernel_exec_params_t{copy_kernels[0], src_nv12_image, out_y_plane,  src_nv12_desc.image_width / 2,            src_nv12_desc.image_height / 2,            comp_t::Y_ONLY},
            kernel_exec_params_t{copy_kernels[1], src_nv12_image, out_uv_plane, work_units(src_nv12_desc.image_width, 4), work_units(src_nv12_desc.image_height, 4), comp_t::UV_ONLY},
            kernel_exec_params_t{copy_kernels[2], src_nv12_image, out_y_plane,  work_units(src_nv12_desc.image_width, 4), src_nv12_desc.image_height,                comp_t::Y_ONLY},
            kernel_exec_params_t{copy_kernels[3], src_nv12_image, out_uv_plane, work_units(src_nv12_desc.image_width, 4), src_nv12_desc.image_height / 2,            comp_t::UV_ONLY},
            kernel_exec_params_t{copy_kernels[4], src_uv_plane,   out_uv_plane, work_units(src_nv12_desc.image_width, 4), work_units(src_nv12_desc.image_height, 4), comp_t::UV_ONLY},
            kernel_exec_params_t{copy_kernels[5], src_y_plane,    out_y_plane,  work_units(src_nv12_desc.image_width, 4), src_nv12_desc.image_height,                comp_t::Y_ONLY},
    };
    ...
    for (size_t i = 0; i < kernel_execution_params.size(); ++i)
    {
        ...
        cl_mem    dst_plane   = kernel_execution_params[i].dst_plane;
        ...
        err = clSetKernelArg(copy_kernel, 1, sizeof(dst_plane), &dst_plane);
        if (err != CL_SUCCESS)
        {
            std::cerr << "On iteration " << i << ":\n";
        ...
        switch(kernel_execution_params[i].comp_type)
        {
            case comp_t::Y_ONLY:
            {
                comparison_map_region[0] = src_nv12_desc.image_width;
                comparison_map_region[1] = src_nv12_desc.image_height;
                comparison_plane         = src_y_plane;
                comparison_width         = src_nv12_desc.image_width;
                comparison_height        = src_nv12_desc.image_height;
                break;
            }
            case comp_t::UV_ONLY:
            {
                comparison_map_region[0] = src_nv12_desc.image_width / 2;
                comparison_map_region[1] = src_nv12_desc.image_height / 2;
                comparison_plane         = src_uv_plane;
                comparison_width         = src_nv12_desc.image_width;
                comparison_height        = src_nv12_desc.image_height / 2;
                break;
            }
            default:
            {
